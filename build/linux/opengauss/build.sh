...
function print_help()
{
    ...
    -m|--version_mode      this values of paramenter is Debug, Release, Memcheck, DMSTest, MemcheckTest. The default value is Release.
...
if [ ! "$version_mode"x == "Debug"x ] && [ ! "$version_mode"x == "Release"x ] && [ ! "$version_mode"x == "DMSTest"x ] && [ ! "$version_mode"x == "Memcheck"x ] && [ ! "$version_mode"x == "MemcheckTest"x ]; then
...
cp -r $LIB_PATH/lz4/comm/lib                   $DMS_LIBRARYS/lz4/lib
...
cp -r $LIB_PATH/lz4/comm/include                  $DMS_LIBRARYS/lz4/include
...
if [ "$build_tool"x == "cmake"x ];then
    ...
    elif [ "$version_mode"x == "MemcheckTest"x ];then
