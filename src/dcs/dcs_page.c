    ...
    dms_buf_ctrl_t *ctrl, dms_message_t *msg, dms_lock_mode_t mode)
{
    ...
    g_dms.callback.update_node_lfn(dms_ctx->db_handle, ack->node_id, ack->node_lfn, node_data,
        node_data_len);
#endif
    ...
    uint8 master_id, uint8 *owner_id)
{
    ...
    DDES_FAULT_INJECTION_CALL(DMS_FI_REQ_TRY_ASK_MASTER_FOR_PAGE_OWNER_ID, MSG_REQ_TRY_ASK_MASTER_FOR_PAGE_OWNER_ID);
    ...
    LOG_DEBUG_INF("[DCS][%s][try ask master for page owner id]%s: src_id=%d, dst_id=%d, flag=%d, lock_mode=%d",
