...
static bool8 dls_request_spin_lock(dms_context_t *dms_ctx, dms_drid_t *lock_id, uint32 timeout_ticks)
{
    uint32 spin_times = 0;
    uint32 wait_ticks = 0;

    do {
        if (dls_request_lock(dms_ctx, lock_id, DMS_LOCK_NULL, DMS_LOCK_EXCLUSIVE) == DMS_SUCCESS) {
            return CM_TRUE;
        }
...
{
    CM_ASSERT(dlock->drid.type > DMS_DR_TYPE_INVALID && dlock->drid.type < DMS_DR_TYPE_MAX);
    ...
    do {
        ...
        if (!is_owner && !dls_request_spin_lock(dms_ctx, &dlock->drid, 1)) {
            drc_unlock_local_resx(lock_res);
...
static int32 dls_do_spin_try_lock(void *dms_ctx, dms_drlock_t *dlock)
{
    bool8 is_locked;
    bool8 is_owner;
    if (SECUREC_UNLIKELY(dlock->drid.type == DMS_DR_TYPE_INVALID || dlock->drid.type >= DMS_DR_TYPE_MAX)) {
    ...
    if (drc_try_lock_local_resx(lock_res)) {
        ...
        if (!is_owner) {
            int32 ret = dls_try_request_lock(dms_ctx, &dlock->drid, DMS_LOCK_NULL, DMS_LOCK_EXCLUSIVE);
            if (ret != DMS_SUCCESS) {
