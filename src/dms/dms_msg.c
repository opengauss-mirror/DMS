...
void cm_send_error_msg(dms_message_head_t *head, int32 err_code, char *err_info)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_ACK_ERROR, MSG_ACK_ERROR);
    ...
    uint8 type, dms_session_e sess_type, uint8 owner_id)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_REQ_INVALID_OWNER, MSG_REQ_INVALID_OWNER);
    ...
    dms_lock_mode_t req_mode)
{
    ...
    int32 ret = drc_request_page_owner(dms_ctx->inst_id, dms_ctx->sess_id, dms_ctx->resid, dms_ctx->len,
        dms_ctx->type, &req_info, &result);
    ...
    dms_lock_mode_t curr_mode, dms_lock_mode_t req_mode)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_REQ_ASK_MASTER_FOR_PAGE, MSG_REQ_ASK_MASTER_FOR_PAGE);
...
static int32 dms_send_ask_res_owner_id_req(dms_context_t *dms_ctx, uint8 master_id, uint64 *ruid)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_REQ_ASK_RES_OWNER_ID, MSG_REQ_ASK_RES_OWNER_ID);
    ret = mfc_send_data(&req.head);
    *ruid = req.head.ruid;
    return ret;
}
...
static int dms_notify_owner_for_res_r(dms_process_context_t *ctx, dms_res_req_info_t *req_info)
{
    ...
    if (req_info->owner_id != req_info->req_id) {
        ...
        DMS_FAULT_INJECTION_CALL(DMS_FI_REQ_ASK_OWNER_FOR_PAGE, MSG_REQ_ASK_OWNER_FOR_PAGE);
...
void dms_proc_ask_master_for_res(dms_process_context_t *proc_ctx, dms_message_t *receive_msg)
{
    ...
    int ret = drc_request_page_owner(proc_ctx->inst_id, proc_ctx->sess_id, req.resid, req.len, req.res_type,
        req_info, &result);
    if (SECUREC_UNLIKELY(ret != DMS_SUCCESS)) {
...
void dms_proc_ask_res_owner_id(dms_process_context_t *proc_ctx, dms_message_t *receive_msg)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_ACK_ASK_RES_OWNER_ID, MSG_ACK_ASK_RES_OWNER_ID);
    if (mfc_send_data(&ack.head) == DMS_SUCCESS) {
...
void dms_proc_invld_req(dms_process_context_t *proc_ctx, dms_message_t *receive_msg)
{
    ...
    if (req.invld_owner) {
        ...
        DMS_FAULT_INJECTION_CALL(DMS_FI_ACK_INVLD_OWNER, MSG_ACK_INVLD_OWNER);
    } else {
        ack_cmd = MSG_ACK_INVLDT_SHARE_COPY;
        DMS_FAULT_INJECTION_CALL(DMS_FI_ACK_INVLDT_SHARE_COPY, MSG_ACK_INVLDT_SHARE_COPY);
    }
...
static int32 dms_smon_send_confirm_req(res_id_t *res_id, drc_request_info_t *cvt_req, uint64 *ruid)
{
    ...
    DMS_FAULT_INJECTION_CALL(DMS_FI_REQ_CONFIRM_CVT, MSG_REQ_CONFIRM_CVT);
    ...
    drc_request_info_t *cvt_req)
{
    ...
    (void)drc_cancel_converting(buf_res, cvt_req, &cvt_info);
...
static bool8 dms_check_page_ownership_r(dms_context_t *dms_ctx, uint8 master_id, dms_lock_mode_t curr_mode)
{
    ...
    dms_common_ack_t ack = *(dms_common_ack_t*)msg.buffer;
    mfc_release_response(&msg);
    LOG_DEBUG_INF("[DMS][%s]: check ownership result:%d", cm_display_resid(req.resid, DRC_RES_PAGE_TYPE), ack.ret);
    return ack.ret;
}
