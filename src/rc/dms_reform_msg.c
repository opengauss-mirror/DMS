...
void dms_reform_init_req_sync_share_info(dms_reform_req_sync_share_info_t *req, uint8 dst_id)
{
    ...
    MES_INIT_MESSAGE_HEAD(&(req->head), MSG_REQ_SYNC_SHARE_INFO, 0, g_dms.inst_id, dst_id, reform_context->sess_judge,
        CM_INVALID_ID16);
...
void dms_reform_init_req_sync_step(dms_reform_req_sync_step_t *req)
{
    ...
    MES_INIT_MESSAGE_HEAD(&(req->head), MSG_REQ_SYNC_STEP, 0, g_dms.inst_id, share_info->reformer_id, ctx->sess_proc,
        CM_INVALID_ID16);
...
void dms_reform_init_req_prepare(dms_reform_req_prepare_t *req, uint8 dst_id)
{
    ...
    MES_INIT_MESSAGE_HEAD(&(req->head), MSG_REQ_REFORM_PREPARE, 0, g_dms.inst_id, dst_id, ctx->sess_judge,
        CM_INVALID_ID16);
...
void dms_reform_init_req_sync_next_step(dms_reform_req_sync_step_t *req, uint8 dst_id)
{
    ...
    MES_INIT_MESSAGE_HEAD(&(req->head), MSG_REQ_SYNC_NEXT_STEP, 0, g_dms.inst_id, dst_id, ctx->sess_proc,
        CM_INVALID_ID16);
    req->head.size = (uint16)sizeof(dms_reform_req_sync_step_t);
    req->head.rsn = mes_get_rsn(ctx->sess_proc);
    req->curr_step = reform_info->current_step;
...
{
    MES_INIT_MESSAGE_HEAD(&req->head, MES_REQ_MGRT_MASTER_DATA, 0, g_dms.inst_id, migrate_task->import_inst,
...
int dms_reform_req_migrate_res(migrate_task_t *migrate_task, uint8 type)
{
    ...
    int ret = DMS_SUCCESS;
    bilist_t *res_list = NULL;
    drc_buf_res_t *buf_res = NULL;
    drc_res_ctx_t *ctx = DRC_RES_CTX;
    
    if (type == DRC_RES_PAGE_TYPE) {
        res_list = &ctx->global_buf_res.res_parts[migrate_task->part_id];
    } else {
        res_list = &ctx->global_lock_res.res_parts[migrate_task->part_id];
    }
...
{
    drc_buf_res_t *buf_res = drc_get_buf_res(res_msg->resid, res_msg->len, type, CM_FALSE, CM_TRUE);
    if (buf_res == NULL) {
    ...
    uint8 master_id)
{
    ...
    if (req_rebuild == NULL) {
        ...
        MES_INIT_MESSAGE_HEAD(&req_rebuild->head, MSG_REQ_BUF_RES_DRC_REBUILD, 0, dms_ctx->inst_id, master_id,
            g_dms.reform_ctx.sess_proc, CM_INVALID_ID16);
...
void dms_reform_proc_req_rebuild_buf_res(dms_process_context_t *ctx, mes_message_t *receive_msg)
{
    ...
    while (offset < req_rebuild->offset) {
        ...
        ctrl = (dms_buf_ctrl_t *)((uint8 *)req_rebuild + offset);
        offset += (uint32)sizeof(dms_buf_ctrl_t);

        lsn = *(uint64 *)((uint8 *)req_rebuild + offset);
        offset += (uint32)sizeof(uint64);

        is_dirty = *(bool8 *)((uint8 *)req_rebuild + offset);
        offset += (uint32)sizeof(bool8);

        ret = dms_reform_rebuild_buf_res_l(pageid, ctrl, lsn, is_dirty, inst_id);
        if (ret != DMS_SUCCESS) {
            LOG_RUN_ERR("[DRC]drc_rebuild_buf_res_l,myid:%u, pageid:%s", g_dms.inst_id, cm_display_pageid(pageid));
            break;
        }
    }
...
void dms_reform_init_req_res(dms_reform_req_res_t *req, uint8 type, char *pageid, uint8 dst_id, uint32 action)
{
    ...
    MES_INIT_MESSAGE_HEAD(&(req->head), MSG_REQ_PAGE, 0, g_dms.inst_id, dst_id, ctx->sess_proc, CM_INVALID_ID16);
...
int dms_reform_req_msg_sync(void)
{
    ...
    uint8 inst_id = (uint8)g_dms.inst_id;

    MES_INIT_MESSAGE_HEAD(&req, MSG_REQ_MSG_SYNC, 0, inst_id, inst_id, ctx->sess_proc, CM_INVALID_ID16);
    req.size = (uint16)sizeof(mes_message_head_t);
    req.rsn = mes_get_rsn(ctx->sess_proc);

    mes_get_queue_count(&reform_info->msg_sync_total_count);
    if (reform_info->msg_sync_total_count == 0) {
        return DMS_SUCCESS;
    } else {
        return mes_send_inter_msg_all_queue(&req);
    }
}
...
{
    reform_info_t *reform_info = DMS_REFORM_INFO;
    (void)cm_atomic32_inc(&reform_info->msg_sync_count);
    mfc_release_message_buf(receive_msg);
}
...
void dms_reform_init_req_switchover(dms_reform_req_switchover_t *req, uint16 sess_id)
{
    ...
    MES_INIT_MESSAGE_HEAD(&req->head, MSG_REQ_SWITCHOVER, 0, g_dms.inst_id, reform_info->reformer_id, sess_id,
        CM_INVALID_ID16);
...
void dms_reform_init_channel_check(mes_message_head_t *head, uint8 inst_id, uint32 index, uint16 sess_id)
{
    ...
    MES_INIT_MESSAGE_HEAD(head, MSG_REQ_CHANNEL_CHECK, 0, g_dms.inst_id, inst_id, index, sess_id);
...
static void dms_reform_ack_channel_check(mes_message_head_t *src_head)
{
    ...
    mes_init_ack_head(src_head, &head, MSG_ACK_COMMON, sizeof(mes_message_head_t), src_head->src_sid);
...
{
    reform_context_t *reform_context = DMS_REFORM_CONTEXT;
    MES_INIT_MESSAGE_HEAD(head, MSG_REQ_CHECK_REFORM_DONE, 0, g_dms.inst_id, dst_id, reform_context->sess_proc,
                          CM_INVALID_ID16);
